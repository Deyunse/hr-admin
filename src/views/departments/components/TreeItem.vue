...
export default {
  ...
  data () {
    const validateName = async (rule, value, callbanck) => {
      const res = await getDepartmentList()

      if (this.isEdit) {
        // 标题不存在编辑
        const pid = this.node.data.pid // 把当前要编辑的部门父级找到了 然后根据这个pid 去找所有的兄弟元素
        res.depts.filter(item => item.pid === pid && item.id !== this.node.data.id).some(item => item.name === value) ? callbanck(new Error('部门名字重复')) : callbanck()
      } else {
        const id = this.node.data ? this.node.data.id : ''
        res.depts.filter(item => item.pid === id).some(item => item.name === value) ? callbanck(new Error('部门名字重复')) : callbanck()
      }
    }
    return {
